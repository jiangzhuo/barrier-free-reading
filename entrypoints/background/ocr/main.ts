...
// @ts-ignore
import * as cvInit from './opencv/opencv.js'

let ort: typeof import("onnxruntime-common");

import {AsyncType, data2canvas, int, resizeImg, SessionType, toPaddleInput} from "./untils";

let cv: any;
export {ocr, init};

let dev = true;
let det: SessionType, rec: SessionType, dic: string[];
let limitSideLen = 960,
    imgH = 48,
    imgW = 320;
let detShape = [NaN, NaN];
...
/** 主要操作 */
async function ocr(img: ImageData) {
    console.time();
    let h = img.height,
        w = img.width;
...
async function runDet(transposedData: number[][][], image: ImageData, det: SessionType) {
    ...
    let detFeed: { [key: string]: TypedTensor<"float32"> } = {};
...
async function runRec(b: number[][][][], imgH: number, imgW: number, rec: SessionType) {
    ...
    let recFeed: { [key: string]: TypedTensor<"float32"> } = {};
...
function beforeDet(image: ImageData, shapeH: number, shapeW: number) {
    ...
    if (dev) {
        ...
        // todo show this offscreen canvas somewhere
    }
...
function afterDet(data: AsyncType<ReturnType<typeof runDet>>["data"], w: number, h: number, srcData: ImageData) {
    ...
    for (let i = 0; i < contours.size(); i++) {
        ...
        edgeRect.push({box, img: context.getImageData(0, 0, c.width, c.height)});
    }
...
function getRotateCropImage(img: OffscreenCanvas, points: BoxType) {
    ...
    let width = dst_rot ? dst_rot.matSize[1] : dst_img_width;
    let height = dst_rot ? dst_rot.matSize[0] : dst_img_height;
